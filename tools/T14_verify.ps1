$ErrorActionPreference = "Stop"

Write-Host "INICIANDO VERIFICACAO DO T14 (Open Graph Cards & Metadata)" -ForegroundColor Cyan
Write-Host "================================================================"

$reportFile = "reports\$(Get-Date -Format 'yyyy-MM-dd-HHmm')-T14-og-cards-final.md"
New-Item -ItemType Directory -Force -Path "reports" | Out-Null
Set-Content -Path $reportFile -Value "# Relatório de Verificacao T14 (Open Graph & Metadata)`n"

function Append-Report($text) {
    Add-Content -Path $reportFile -Value $text
    Write-Host $text
}

Append-Report "## Objetivo`nVerificar presenca massiva do SEO Dinamico gerado pelo next/og nas rotas de indices unicos.`n"

# 1. Checar arquivos chave
$filesToCheck = @(
    "lib\og\og-theme.ts",
    "lib\og\og-utils.ts",
    "lib\og\og-types.ts",
    "docs\T14_OG_CARDS.md",
    "app\snapshots\transparencia\[id]\opengraph-image.tsx",
    "app\snapshots\territorio\[id]\opengraph-image.tsx",
    "app\snapshots\diffs\[id]\opengraph-image.tsx",
    "app\alertas\[id]\page.tsx",
    "app\alertas\[id]\opengraph-image.tsx"
)

Append-Report "## DIAG e VERIFY de Arquivos OGs`n"
$missingFiles = 0
foreach ($f in $filesToCheck) {
    if (Test-Path -LiteralPath $f) {
        Append-Report "- [x] $f presente"
    }
    else {
        Append-Report "- [ ] $f AUSENTE"
        $missingFiles++
    }
}

# 2. Executar Linter
Append-Report "`n## Testes Estaticos: Linter (Strict)`n"
Write-Host "Rodando linter..." -ForegroundColor Yellow
$lintOk = $true
try {
    npm run lint 2>&1 | Out-Host
    Append-Report "- [x] Linter passou sem problemas."
}
catch {
    $lintOk = $false
    Append-Report "- [ ] Linter **falhou**."
}

# 3. Executar Typecheck
Append-Report "`n## Testes Estaticos: Typecheck`n"
Write-Host "Rodando typecheck..." -ForegroundColor Yellow
$tcOk = $true
try {
    npm run typecheck 2>&1 | Out-Host
    Append-Report "- [x] Typecheck passou sem problemas."
}
catch {
    $tcOk = $false
    Append-Report "- [ ] Typecheck **falhou**."
}

# 4. Executar Build
Append-Report "`n## Testes Dinamicos: Build (SSR Node JS)`n"
Write-Host "Gerando endpoints Edge API para imagens em memoria..." -ForegroundColor Yellow
$buildOk = $true
try {
    npm run build 2>&1 | Out-Host
    Append-Report "- [x] Build OK."
}
catch {
    $buildOk = $false
    Append-Report "- [ ] Build **falhou**."
}

# 5. Fechamento
Append-Report "`n## Leitura Fria do Estado Atual`n"
Append-Report "- Imagens dinâmicas para alertas e snapshots rodam nativamente Edge na Vercel ou Node puro sem Canvas/Cairo packages."
Append-Report "- Metadados de OG injetados com fallback para Base URL do Env."

Append-Report "`n## NEXT`n"
Append-Report "- **T14b:** Metadatas para telas agregadas."
Append-Report "- **T15:** Payload nativo Slack/Discord."

if ($missingFiles -eq 0 -and $lintOk -and $tcOk -and $buildOk) {
    Append-Report "`n✅ RESULTADO: **PASSOU**"
    Write-Host "`n==== T14 VERIFICADO COM SUCESSO ====" -ForegroundColor Green
}
else {
    Append-Report "`n❌ RESULTADO: **FALHOU**"
    Write-Host "`n==== T14 TEM FALHAS ====" -ForegroundColor Red
}

Write-Host "Relatório salvo em: $reportFile"
