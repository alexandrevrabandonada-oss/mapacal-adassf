            ...
            <div style={{ background: OG_THEME.colors.background, width: "100%", height: "100%", display: "flex", flexDirection: "column", fontFamily: OG_THEME.fonts.sans, padding: 64 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", flexGrow: 1 }}>
                    ...
                    <h1 style={{ fontSize: 64, fontWeight: 900, lineHeight: 1.1, color: OG_THEME.colors.foreground, marginBottom: 24, letterSpacing: -2 }}>
                        {truncateText(snapshot.title || "Agregado Cidadão de Transparência Urbana", 80)}
                    </h1>

                    {metrics ? (
                        <div style={{ display: "flex", gap: 32, marginTop: 16 }}>
                            <div style={{ display: "flex", flexDirection: "column" }}>
                                <span style={{ fontSize: 24, color: OG_THEME.colors.neutral.text }}>Publicados</span>
                                <span style={{ fontSize: 48, fontWeight: 'bold' }}>{metrics.total_published || 0}</span>
                            </div>
                            <div style={{ display: "flex", flexDirection: "column" }}>
                                <span style={{ fontSize: 24, color: OG_THEME.colors.neutral.text }}>Verificados</span>
                                <span style={{ fontSize: 48, fontWeight: 'bold' }}>{metrics.total_verified || 0}</span>
                            </div>
                            <div style={{ display: "flex", flexDirection: "column" }}>
                                <span style={{ fontSize: 24, color: OG_THEME.colors.neutral.text }}>Bloqueados</span>
                                <span style={{ fontSize: 48, fontWeight: 'bold', color: OG_THEME.colors.danger.text }}>{metrics.total_blocked || 0}</span>
                            </div>
                        </div>
                    ) : (
                        <p style={{ fontSize: 32, color: OG_THEME.colors.neutral.text }}>
                            Captura materializada das dinâmicas predatórias e de impacto em calçadas do município.
                        </p>
                    )}
                </div>
