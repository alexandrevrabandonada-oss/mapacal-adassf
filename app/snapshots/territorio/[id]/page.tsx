...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!isSupabaseConfigured()) return { title: 'Snapshot Território' };

  const { snapshot } = await getPublicSnapshotById(id);

  if (!snapshot) return { title: 'Snapshot Território não encontrado' };

  const cleanTitle = snapshot.title || "Snapshot Território";
  const desc = `Fotografia pública priorizada por bairros gerada na janela de ${(snapshot.data as any)?.time_window_days || '?'} dias.`;
  ...
  return {
    ...
    openGraph: {
      title: cleanTitle,
      description: desc,
      url: `${baseUrl}/snapshots/territorio/${id}`,
...
export default async function SnapshotTerritorioPage({ params }: PageProps) {
  const { id } = await params;
  const publicPath = `/snapshots/territorio/${id}`;

  const result = await getPublicSnapshotById(id);

  if (!result.ok || !result.snapshot) {
    return (
      <SiteShell title="Snapshot Territorio" subtitle="Fotografia congelada">
  ...
  return (
    <SiteShell title="Snapshot Territorio" subtitle="Fotografia congelada de prioridades por bairro">
      <div className="space-y-6">
        <SnapshotSummaryCard snapshot={snapshot} />

        <SectionCard title="Dados Territoriais" eyebrow="Sumario">
          {snapshot.data?.summary ? (
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="rounded bg-zinc-50 p-4">
                <p className="text-xs text-zinc-600">Bairros com problemas</p>
                <p className="mt-1 text-2xl font-bold">
                  {String((snapshot.data.summary as Record<string, unknown>).neighborhoods_with_issues || 0)}
                </p>
              </div>
              <div className="rounded bg-zinc-50 p-4">
                <p className="text-xs text-zinc-600">Score de prioridade total</p>
                <p className="mt-1 text-2xl font-bold">
                  {String((snapshot.data.summary as Record<string, unknown>).total_priority_score || 0)}
                </p>
              </div>
            </div>
          ) : (
            <p className="text-sm text-zinc-600">
              Dados detalhados nao disponives. Aplique SQL T09+ para populador snapshot.
            </p>
          )}
        </SectionCard>
        ...
        <SectionCard title="Compartilhar" eyebrow="Links e Texto">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <SharePackDrawer id={id} kind="snapshots/territorio" />
          </div>
          <p className="mt-4 text-xs text-zinc-400">
            O snapshot de território é uma fotografia congelada de prioridades em um momento específico do tempo.
          </p>
        </SectionCard>
      </div>
    </SiteShell>
