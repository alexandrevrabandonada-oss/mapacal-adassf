...
export default function NovoPage() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // Tentar upload de foto se arquivo selecionado
      let photoUploadStatus = "";
      if (selectedFile && payload.reportId) {
        try {
          const formData = new FormData();
          formData.append("file", selectedFile);
          formData.append("reportId", payload.reportId);

          const photoResponse = await fetch("/api/reports/photo/upload", {
            method: "POST",
            body: formData
          });
          ...
          if (photoPayload.ok) {
            photoUploadStatus = " (com foto)";
          } else {
            photoUploadStatus = " (foto falhou)";
          }
        } catch {
          photoUploadStatus = " (foto falhou)";
        }
      ...
      setSubmitMessage((payload.message || "Registro enviado para moderacao") + photoUploadStatus);
      setNeighborhood("");
      setNote("");
      setSelectedTags([]);
      setNearbyItems([]);
  ...
  function handleFileSelect(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!allowedTypes.includes(file.type)) {
      setSubmitError("Tipo de arquivo invalido. Use JPEG, PNG ou WebP.");
      return;
    }

    const maxSize = 8 * 1024 * 1024; // 8 MB
    if (file.size > maxSize) {
      setSubmitError("Arquivo muito grande. Maximo: 8 MB.");
      return;
    }
  ...
  return (
    <SiteShell
      title="Novo registro"
      subtitle="Envie um ponto com localizacao, condicao, contexto e foto (opcional). Foto fica privada e so aparece em reports publicados."
    >
      <SectionCard title="Status do fluxo" eyebrow="T07 ativo">
        <ul className="space-y-2">
          <li>- Fluxo de registro: ativo.</li>
          <li>- Foto privada: ativa (opcional, 8 MB max).</li>
      ...
      {envReady && authState === "authenticated" ? (
        <SectionCard title="Formulario de registro" eyebrow="Dados reais">
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <fieldset>
              <legend className="mb-2 text-xs font-bold uppercase tracking-[0.12em]">Foto (opcional)</legend>
              <p className="mb-2 text-xs text-zinc-600">
                Foto privada, visivel apenas em reports publicados. Evite rostos ou placas de veiculos. Max 8 MB.
              </p>
              <input
                type="file"
                accept="image/jpeg,image/png,image/webp"
                onChange={handleFileSelect}
                className="w-full border-2 border-[var(--ink)] bg-white px-3 py-2 text-sm"
              />
              {photoPreview ? (
                <div className="mt-2">
                  <Image
                    ...
                    width={800}
                    height={600}
                    className="max-h-48 border border-zinc-300"
